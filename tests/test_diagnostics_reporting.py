...
def sample_dataframe():
    ...
    data = {
        "model_name": ["erm", "hirm"],
        "method": ["ERM", "HIRM"],
        "env_config": ["synthetic", "synthetic"],
        "metrics.ISI": [0.8, 0.95],
        "metrics.IG": [0.3, 0.1],
        "metrics.crisis_cvar": [0.6, 0.2],
    }
...
def test_correlations_include_crisis(sample_dataframe):
    ...
    assert "corr(ISI, crisis_cvar)" in correlations
    assert "corr(IG, crisis_cvar)" in correlations
...
def test_summary_multiple_group_keys(sample_dataframe):
    summary = summarize_diagnostics_by_method(
        sample_dataframe, group_by=["method", "env_config"]
    )
    assert {"method", "env_config"}.issubset(summary.columns)
