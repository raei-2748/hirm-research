"""Phase 9 experiment grid runner.

This entrypoint mirrors the paper grid: datasets × methods × seeds with
...
def run_single_experiment(
    ...
    if not force and checkpoint_path.exists() and diagnostics_path.exists():
        print(f"[PHASE9] Skipping completed run dataset={dataset_name} method={method_name} seed={seed}")
        return
    ...
    print(f"[PHASE9] Finished dataset={dataset_name} method={method_name} seed={seed}")


def main() -> None:
    args = parse_args()
    cfg = load_config(args.config)
    raw_methods = _resolve_list(args.methods) or ["erm", "hirm"]
    methods = [(m, METHOD_ALIASES.get(m, m)) for m in raw_methods]
    datasets = _resolve_list(args.datasets) or ["synthetic_heston", "real_spy"]
    seeds = [int(s) for s in (_resolve_list(args.seeds) or list(range(3 if not args.reduced else 1)))]
